...
All PromQL queries rely on labels (cluster, project_id, location, node_name,
namespace, pod, etc.) rather than hardcoded cluster or node names, so alerts
...
# ── GCP console URL templates with dynamic substitutions ──────────────────────
# See: "Annotate notifications with user-defined documentation" docs.[web:34]
_CLUSTER_URL = (
...
def _node_doc(title: str, body: str) -> dict:
    ...
    return {
        ...
        "links": [
            {"display_name": "Cluster Dashboard",     "url": _CLUSTER_URL},
            {"display_name": "Node Details",          "url": _NODE_URL},
            {"display_name": "Monitoring Dashboards", "url": _MONITORING_URL},
        ],
    }
...
def _pod_doc(title: str, body: str) -> dict:
    ...
    return {
        ...
        "links": [
            {"display_name": "Workloads",             "url": _WORKLOADS_URL},
            {"display_name": "Cluster Dashboard",     "url": _CLUSTER_URL},
            {"display_name": "Monitoring Dashboards", "url": _MONITORING_URL},
        ],
    }
...
ALERT_POLICIES = [
    ...
    # Uses GKE system metric kubernetes_io:node_cpu_allocatable_utilization.[web:11]
    (
    ...
    # kubernetes_io:node_memory_allocatable_utilization with aggregation that
    # preserves cluster_name, project_id, location for documentation variables.[web:11][web:83]
    (
    ...
    # Compute utilization from used_bytes / total_bytes for ephemeral storage.[web:52][web:81]
    (
        ...
        (
            ...
            '    }'
            '    /'
            '    kubernetes_io:node_ephemeral_storage_total_bytes{'
            '      monitored_resource="k8s_node"'
            '    }'
            '  ) * 100'
            '  [5m]'
        ...
        _node_doc(
            ...
            "Ephemeral storage utilization on the node has exceeded **90%** based on "
            "used vs. total ephemeral storage.\n\n"
            "**Suggested actions:**\n"
            "- Identify pods writing large amounts of data to ephemeral storage (`kubectl top pods`).\n"
            "- Clear unused images/containers via image GC or `docker system prune`.\n"
            "- Consider migrating stateful workloads to Persistent Volumes."
        ),
    ),

    # ── 6. Pod Memory Utilization > 90% (watched namespaces) ──────────────────
    # working_set_bytes / memory limit per pod from GMP-scraped metrics.
    # These use the default prometheus_target resource; we do not force
    # monitored_resource="k8s_container" so they match correctly.[web:62][web:65]
    (
    ...
    # More than 3 restarts in a 15-minute window signals a crash-loop.[web:48][web:49]
    (
...
def create_alert_policy(
    ...
    condition = monitoring_v3.AlertPolicy.Condition(
        ...
        condition_prometheus_query_language=pql_condition,
    )
